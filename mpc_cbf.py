...
obstacles = [(obs_x, obs_y, obs_r), (obs2_x, obs2_y, obs2_r), (1, 4.5, 1.0), (5, 0, 0.5)]
...
def solve_mpc(x0, y0, theta0):
    ...
    # Control constraints
    opti.subject_to(opti.bounded(0, V, 2))
    opti.subject_to(opti.bounded(-1, Omega, 1))

    # Define CBF constraints for obstacles
    gamma = 0.7  # CBF relaxation parameter (tuning required)
    
    for k in range(N):
        # Compute distance functions for obstacles
        h1 = (X[k] - obs_x)**2 + (Y[k] - obs_y)**2 - (obs_r + safety_margin)**2
        h2 = (X[k] - obs2_x)**2 + (Y[k] - obs2_y)**2 - (obs2_r + safety_margin)**2
        
        # Compute Lie derivatives (change in h)
        Lf_h1 = 2 * (X[k] - obs_x) * V[k] * ca.cos(Theta[k]) + 2 * (Y[k] - obs_y) * V[k] * ca.sin(Theta[k])
        Lf_h2 = 2 * (X[k] - obs2_x) * V[k] * ca.cos(Theta[k]) + 2 * (Y[k] - obs2_y) * V[k] * ca.sin(Theta[k])
        
        # Ensure CBF constraints are satisfied
        opti.subject_to(Lf_h1 + gamma * h1 >= 0)
        opti.subject_to(Lf_h2 + gamma * h2 >= 0)

        # TODO: put conatraints for sensed obstacles
        _, obstacle_clusters = simulate_lidar(x0, y0, theta0)
        for cluster in obstacle_clusters:
            center_x, center_y, radius = cluster
            h = (X[k] - center_x)**2 + (Y[k] - center_y)**2 - (radius + safety_margin)**2
            Lf_h = 2 * (X[k] - center_x) * V[k] * ca.cos(Theta[k]) + 2 * (Y[k] - center_y) * V[k] * ca.sin(Theta[k])
            opti.subject_to(Lf_h + gamma * h >= 0)


    # Cost function: Prioritize reaching the goal while minimizing effort
    goal_weight = 150  
    effort_weight = 0.2  
...
ax.legend()
ax.grid()
ax.set_title("MPC-CBF for Unicycle Model with Obstacles")
